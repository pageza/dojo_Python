###### Optional Assignment: The Requests Library ########
#  Build a simple program that makes use of the requests module.
#
#  In addition to including a lot of great features in the language itself,
#  there are many modules available for install via pip that are a lot of fun to work with.
#  We talked a little about pip before, so you should have the basic idea that pip
#  exists so that developers can use code other people have written.
#
#  One of those code libraries is called requests. It allows you to make
#  HTTP requests from a python file and get back a useful response.
#  This library is a simple way to make requests to and get responses from a
#  server without having to set one up yourself. This might remind you a little
#  of using AJAX to interact with API's like you did in web fundamentals.
#  Give it a try and get creative with the data you try to get back.
